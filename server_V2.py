...
# TaskSync - Distributed Collaborative ToDo System
    ...
    while True:
        ...
        try:
            ...
            if discovered_id != SERVER_ID:
                if discovered_id not in known_servers:
                    print(f"Server {discovered_id} entdeckt: {discovered_url}")
...
def add_task(task: str):
    if not is_current_leader():
        return {
            "message": "Dieser Server ist aktuell nicht der Leader. Schreiboperationen sollen vom Leader verarbeitet werden.",
            "server_id": SERVER_ID,
            "leader_id": leader_id
        }
    ...
    return {
        "message": "Task wurde vom Leader hinzugefuegt und repliziert",
        "server_id": SERVER_ID,
        "leader_id": leader_id,
        "tasks": tasks
    }
